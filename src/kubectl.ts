...
export function invoke(command : string, handler? : (result : number, stdout : string, stderr : string) => void) : void {
...
function kubectlInternal(command : string, handler : (result : number, stdout : string, stderr : string) => void) : void {
...
export function checkForKubectl(errorMessageMode : CheckMode, handler : () => void) : void {
    if (kubectlFound) {
        handler();
        return;
    }
...
function checkForKubectlInternal(errorMessageMode : CheckMode, handler : () => void) {
...
function findBinary(binName : string, callback : (exitCode : number | null, output : string) => void) {
...
function getCheckKubectlContextMessage(errorMessageMode : CheckMode) : string {
